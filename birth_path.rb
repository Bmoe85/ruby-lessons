...
def numerology_number(birthdate)
	numerology_number = birthdate[0].to_i + birthdate[1].to_i + birthdate[2].to_i +
	birthdate[3].to_i + birthdate[4].to_i + birthdate[5].to_i + birthdate[6].to_i +
	birthdate[7].to_i 

	numerology_number = numerology_number.to_s
	numerology_number = numerology_number[0].to_i + numerology_number[1].to_i	
	
	if numerology_number > 9 
	numerology_number = numerology_number[0].to_i + numerology_number[1].to_i
	end
...
def get_message(birth_path_number)
	...
	when 1
		message = "One is the leader. The number one indicates the ability to stand alone,
		and is a strong vibration. Ruled by the Sun."
		
	when 2
		message =  "This is the mediator and peace-lover. The number two indicateds the desire
		for harmony. It is a gently, considerate, and sensitive vibration. Ruled by the Moon."
	...
	when 4
		message =  "This is the worker. Practical, with a love of detail, Fours are trustworthy, 
		hard-working, and helpful. Ruled by Uranus."
		
	when 5
		message = "This the freedom lover. The number five is an intellectual vibration. These are
	...
	when 8
		message = "This is the manager. Number Eight is a strong, successful, and material vibration. Ruled
		by Saturn."
	...
	when 9
		...
		message = "Uh no! Your birth path number is not 1-9!"
	end	
end	
...
birth_path_number = numerology_number(birthdate)
